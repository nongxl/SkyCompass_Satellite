...
public:
    ...
    static bool getTLE(int noradId, TLEData& outTle, uint32_t maxAgeSeconds = 2 * 24 * 3600, WiFiClientSecure* sharedClient = nullptr);
...
private:
    ...
    static bool fetchFromNetwork(int noradId, TLEData& outTle, WiFiClientSecure* sharedClient = nullptr);
};
