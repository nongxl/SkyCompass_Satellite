...
| **`S`** | 卫星选择与百科全书 | `f` 分类筛选，`d` 删除自选卫星，`O` 发射单星入轨 |
...
| **`Ctrl`** | 射频终端控制台 (RF Console) | 实时多普勒频移监控、背景动态瀑布图与空间遥测抓包；`T` 测试注入 |
...
| **`Tab`** | 全局切换视觉模式 | 循环切换普通/极光/红光夜视护眼模式（在 RF 控制台同样生效） |
...
- **空间遥测与地面站灵感**：感谢 [TinyGS](https://tinygs.com/) 开放硬件与全球分布式地面站网络在业余空间 LoRa 遥测接收与多普勒调谐方面的先驱探索与宝贵灵感。
