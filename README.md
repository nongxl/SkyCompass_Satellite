...
用户可根据手头的硬件模块自由选择以下接线组合，各场景均已在系统固件中经过总线调度与引脚适配：

| 硬件组合方案 | 顶部 14-Pin 扩展槽 | Cap 顶部 HY2.0-4P 接口 | 机身侧面 Grove 接口 | 硬件特性与适用场景 |
| :--- | :--- | :--- | :--- | :--- |
| **Cap + 浑仪云台 + 像素副屏** | **Cap LoRa-1262** (GNSS+射频) | **Unit 8Servos** (I2C: G8/G9) | **Chain Mono** (UART: G1/G2) | **全功能组合**：双频定位、遥测抓包、浑仪机械三轴指向与 8x8 副屏动态外显全部激活，多总线独立并发无冲突 |
| **Cap + 浑仪机械云台** | **Cap LoRa-1262** (GNSS+射频) | **Unit 8Servos** (I2C: G8/G9) | 闲置 / 备用 | 空间三轴物理追踪，天球拱门立体复现过境走向、最高仰角与卫星滑行轨迹 |
| **Cap + 像素副屏** | **Cap LoRa-1262** (GNSS+射频) | 闲置 / 备用 | **Chain Mono** (UART: G1/G2) | 便携高精度双频授时定位、LoRa 空间遥测监听与 8x8 像素动态过境动画外显 |
| **仅 Cap 顶置单独使用** | **Cap LoRa-1262** (GNSS+射频) | 闲置 / 备用 | 闲置 / 备用 | **手持便携**：无任何外挂线缆，整机极致紧凑，提供高精度双频 GNSS 与 433~438MHz 空间无线电抓包 |
| **外置 Unit GPS 徒步便携版** | 未插接 Cap | - | **Unit GPS v1.1** (UART 独占) | 侧面外接高灵敏度陶瓷天线 GNSS 模块，适合户外野外无网络环境单兵观测 |
| **独立浑仪舵机云台 (无Cap)** | 未插接 Cap | - | **Unit 8Servos** (I2C: G2/G1) | 侧面直连舵机驱动板，依靠 WiFi 网络授时或断网离线预存坐标驱动浑仪 |
| **独立像素副屏 (无Cap)** | 未插接 Cap | - | **Chain Mono** (UART: G1/G2) | 侧面直连 8x8 点阵副屏，依靠 WiFi 网络授时或断网坐标渲染过境动画 |
| **纯单机模拟演示 (无外设)** | 无外接硬件 | - | 闲置 | 零外设门槛脱机运行，通过 WiFi NTP 对时与内置离线坐标，或按 `C` 键手动设定观测点 |

> 💡 **外设配置向导**：开机后在任意主界面按下 **`M`** 键，可随时呼出图形化硬件向导（Hardware Wizard），自由勾选模块并由系统实时校验总线冲突与显示动态接线指引。  
> ⚠️ **舵机供电规范**：驱动三轴浑仪乐高舵机时，Unit 8Servos 绿色接线端子**必须接入 5V DC 独立外接电源**（峰值电流可达 1.5A），严禁仅依靠 Cardputer 主控带载 3 个舵机，以防电压跌落导致 ESP32 重启。  
> 🔌 **Chain Mono 接线**：副屏模块具有 IN 与 OUT 两个接口，Grove 线缆必须插接至 **IN 端口**（插 OUT 端口将无法正常通信）。
...
├── docs/                         # 专题技术白皮书与硬件设计文档
├── scripts/                      # PC 端配套工具与数据预处理工具链
│   ├── update_builtin_tles.py    # 出厂 TLE 轨道自动同步与固件头文件烘焙脚本
│   ├── get_screenshot.py         # 串口原始像素流高保真截图接收与无损重构
│   ├── gen_light_points.py       # NASA Black Marble 夜景灯光点云离线重采样
│   └── optimize_earth_data.py    # 全球陆地矢量轮廓离线三角函数预计算
└── platformio.ini                # PlatformIO 编译与依赖库配置文件
```

### 固件打包与出厂轨道数据更新 (scripts/update_builtin_tles.py)

为确保首次烧录后设备在**完全离线**环境下即开即用（无需联网即可精确计算全部卫星过境），并彻底杜绝首次开机因本地无缓存而连续向 CelesTrak 发起密集请求触发 IP 封锁（HTTP 403），本项目开发了出厂 TLE 一键自动同步与固件烘焙脚本 `scripts/update_builtin_tles.py`：

```bash
# 步骤 1：一键拉取全部 59 颗预置卫星当天的最新 TLE 并自动烘焙至 src/core/builtin_tles.h
...
- **全量目标自动解析**：静态扫描并解析 `src/core/encyclopedia.cpp`，自动提取百科库中全部 59 颗预置卫星（天宫、空间站、哈勃、导航群、业余无线电卫星、深空望远镜等）的 NORAD ID 与元数据；
- **双通道容灾与动力学拟合**：优先拉取 CelesTrak 官方 GP API，遇到网络限制时自动无缝回退至开源卫星轨道镜像源；针对 JWST、鹊桥等非地心深空飞行器自动应用专有轨道动力学伪 TLE 拟合，实现 100% 轨根覆盖率；
- **固件静态内嵌烘焙**：自动生成只读头文件 `src/core/builtin_tles.h`，包含完整的 TLE 数据结构表与精确到秒的 UTC 烘焙时间戳；
- **首次开机自动建表 (Cache Seeding)**：新固件首次烧录启动时，系统自检发现 LittleFS 闪存为空，会秒级将出厂烘焙的最新 TLE 注入本地文件系统。后续设备开机或联网时，直接复用有效的本地缓存，无需重复高频向云端请求，从源头上保障网络健壮性。
