...
* **多普勒偏差值（括号中的数字）**：在 RX 频率右侧的括号中（例如 `(+1.5k)` 或 `(-3.2k)`）表示当前的**实时多普勒频移量 $\Delta f$（单位：kHz）**。
  * **多普勒频移物理模型**：
    由于卫星以极高速度（第一宇宙速度，约 $7.8\text{ km/s}$）相对于地面观测者运动，观测者接收到的无线电频率会发生偏移。频移 $\Delta f$ 计算公式如下：
    $$\Delta f = -f_0 \times \frac{v_{radial}}{c}$$
    其中 $f_0$ 为卫星无线电的中心频率，$c$ 为光速（约 $299792.458\text{ km/s}$），$v_{radial}$ 为卫星相对于观测者的**径向速度**（向观测者靠近时 $v_{radial} < 0$，频移 $\Delta f > 0$，表现为频率升高；远离观测者时 $v_{radial} > 0$，频移 $\Delta f < 0$，表现为频率降低）。
  * **通联指导意义**：
